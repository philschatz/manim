...
class PrincipleComponent(ThreeDScene):
    def construct(self):
        ...
        t0b= MathTex(r"z(x,y) = x^2-y^2").scale(0.5).next_to(t0,DOWN).set_color(GREEN)
        t1= TextMobject("Definition of the Gaussian Curvature:").scale(0.5).next_to(t0b,DOWN)
        ancor= t1.copy()
        t2= MathTex(r"K = \frac{1}{R_1} \cdot \frac{1}{R_2}").scale(0.5).next_to(t1,DOWN)
        ...
        t5= MathTex(r"R_1 = 0.5").scale(0.5).next_to(t4,DOWN).align_to(ancor,LEFT)
        t6= MathTex(r"R_2 = -0.5").scale(0.5).next_to(t5,DOWN).align_to(ancor,LEFT)
        t7= MathTex(r"\rightarrow K = -4").scale(0.5).next_to(t6,DOWN).align_to(ancor,LEFT)
        t8= TextMobject("Principle curvatures have different signs").scale(0.5).next_to(t7,DOWN).align_to(ancor,LEFT)
        t9 =MathTex(r"\rightarrow \text{Gaussian curvature negative}").scale(0.5).next_to(t8,DOWN).align_to(ancor,LEFT)
        t10=MathTex(r"\rightarrow \text{Point (0,0) is a saddle point}").scale(0.5).next_to(t9,DOWN).align_to(ancor,LEFT)
        dot = Dot()
