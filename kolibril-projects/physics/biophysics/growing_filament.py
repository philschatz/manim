...
class No20c(Scene):
    def construct(self):
        ...
        def updat_wdPoff(mob,dt):
            ...
            mob.time += 1/config["frame_rate"]    # one second
        ...
        ## equation
        eq1 = MathTex(r"\frac{dn^+}{dt} = "," - k^+_{off}", " + c_A \cdot k^+_{on}")
        #eq1.submobjects[2].set_style(fill_opacity=0)
        self.add(eq1.to_edge(DL))

        #eq
        ec1=MathTex(r"c_A >  \frac{k^+_{off}}{k^+_{on}}  \rightarrow \text{growing filament}")
        ec1.next_to(sq, DOWN)
