...
class Fermat(Scene):
    ...
    def construct(self):
        # box
        ground = Polygon(DOWN+2*LEFT,DOWN*2+2*LEFT, DOWN*2+2*RIGHT,DOWN+2*RIGHT, fill_opacity=1)
        ground.set_style(stroke_color=GRAY, fill_color=WHITE)
        ##
        r1=4
        r2=5
        angle_num=TAU/8
        ...
        A_text = MathTex("P_1(x_1, y_1)" , color= BLUE).next_to(point_1, DOWN)
        R = Dot(point_R, color = BLUE)
        R_text = MathTex("R(x,0)" , color= BLUE).next_to(point_R, DOWN)

        B = Dot(point_2)
        B_text = MathTex("P_2(x_2, y_2)" , color= BLUE).next_to(point_2, DOWN)
...
if __name__ == "__main__":
    ...
    os.system(f"manim   --custom_folders  --disable_caching -p -c '#343434' --config_file '{project_path}/manim_settings.cfg' " + script_name)