...
class Zeemann1(Scene):
    def construct(self):
        ...
        bo=MathTex(r"\vec{B}_0 > 0").set_color( BLACK)
        bo.next_to(bgArrows.submobjects[0], DOWN,buff=0)
        bo.align_to(bgArrows.submobjects[0], LEFT)
        self.add(bo)

        num1 = MathTex(r"{\large \textcircled{\small 1}} ").set_color(BLACK)
        num1.next_to(l1,DOWN,buff=SMALL_BUFF)
        num2 = MathTex(r"{\large \textcircled{\small 2}} ").set_color(BLACK)
        num3 = MathTex(r"{\large \textcircled{\small 3}} ").set_color(BLACK)
...
if __name__ == "__main__":
    ...
    os.system(f"manim   --custom_folders  --disable_caching -p -c '#ece5e1' --config_file '{project_path}/manim_settings.cfg' " + script_name)